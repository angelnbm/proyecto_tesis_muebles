...
Entre la búsqueda de algoritmos para poder cubicar los cortes en la dimensión de la plancha de melamina destacaron 2:
\begin{itemize}
...
\end{itemize}
Dado que se busca una optimización de los cortes en la plancha se opto por el algoritmo Macrects que optimiza dinámicamente los cortes teniendo menos desperdicio que el algoritmo GUillotine.\\
...
En la siguiente figura se muestra la sección del servicio de generación de PDF que construye la tabla de materiales, en donde se recorre el resumen de materiales de la cotización y se agregan las filas con nombre, cantidad y precio unitario usando la librería jsPDF.
\begin{figure}[H]
...
\subsubsection{Retrospectiva}
Al final de esta iteración se revisó el peso asignado y el esfuerzo fue menor al estimado, ya que todos los módulos habían sido diseñados como componentes aislados con interfaces claras. El principal desafío fue coordinar el layout responsivo. La iteración se completó sin pendientes, cerrando el ciclo de desarrollo de todos los módulos de la aplicación.
...
Para evaluar la accesibilidad de la interfaz de Amedida se utilizó la herramienta WAVE (Web Accessibility Evaluation Tool)~\cite{wave}. Esta herramienta analiza la página web directamente en el navegador e identifica problemas según las pautas WCAG 2.1 (Web Content Accessibility Guidelines), clasificando los hallazgos en errores críticos, alertas, elementos de accesibilidad positivos, elementos estructurales, atributos ARIA y errores de contraste de color.
...
El primero y más significativo son los errores de contraste de color, que representan la mayor cantidad de hallazgos en prácticamente todas las vistas. Esto se explica directamente por el uso del tema oscuro en la interfaz de Amedida, en donde los íconos y textos de colores sobre fondos negros o gris muy oscuro no siempre alcanzan la relación de contraste mínima de 4.5:1 que exige el estándar WCAG 2.1.\\
...
Las capturas individuales de la evaluación WAVE para cada vista del sistema se encuentran en el Anexo B.

\chapter{Pruebas}
\label{cap:pruebas}
En este capítulo se presentan los resultados de las pruebas aplicadas al sistema Amedida a lo largo de las ocho iteraciones de desarrollo. Se distinguen dos tipos de evaluación: pruebas unitarias automatizadas ejecutadas con el framework Jest, que validan la lógica interna de cada módulo, y pruebas de caja negra, que verifican el comportamiento funcional del sistema desde la perspectiva del usuario final. Los casos de prueba unitaria PU-01 a PU-27 corresponden a las iteraciones 1 y 2; los casos PU-28 a PU-71 se encuentran en el Anexo A. Los casos de prueba de caja negra PCN-01 a PCN-60 cubren la totalidad de las iteraciones.
...
Las pruebas cubrieron cuatro categorías. Los casos PU-01 a PU-27 verifican la lógica interna de autenticación y diseño de muebles en las iteraciones 1 y 2; los casos PU-28 a PU-71 están en el Anexo A. Los 60 casos de caja negra PCN-01 a PCN-60 evaluaron el sistema desde la perspectiva del mueblista en cada iteración. Las ocho pruebas de aceptación PA-01 a PA-08 recorrieron los flujos completos al cierre de cada iteración. Finalmente, la prueba de usabilidad SUS arrojó un puntaje de 85 puntos, confirmando que la interfaz es aceptable para el usuario objetivo. Todos los casos fueron aprobados.
