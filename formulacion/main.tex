...
\subsubsection{Diseño}
En esta etapa se diseña el diagrama de secuencia de las funcionalidades de la iteración. El flujo principal comienza cuando el mueblista solicita calcular el material desde el canvas, el frontend recorre todas las figuras del diseño, genera la lista de piezas por módulo y calcula cuántas planchas de melamina se requieren para cubrir el área total de cortes.

\subsubsection{Implementación}
Para la implementación de este módulo se desarrolló el archivo \texttt{cubicacion.js} en el frontend, el cual contiene toda la lógica de cálculo. Lo primero que se realizó fue implementar la función \texttt{generateStructuredCuts}, que recorre cada figura del canvas y según su tipo de módulo (cajonera, modular, estante, etc.) genera las piezas correspondientes con sus dimensiones de ancho, alto y cantidad.\\

En el caso de la cajonera, la función calcula automáticamente las piezas de Techo/Piso, Laterales, Frente (uno por cajón) y Fondo a partir de las dimensiones del módulo y la cantidad de cajones configurada. Para el módulo modular, que es el más complejo, calcula adicionalmente las piezas de estantes, divisores y puertas según los parámetros de configuración. Las puertas descuentan 3 mm del ancho y del alto para dejar holgura de montaje.\\

Una vez generadas todas las piezas, se ejecuta el algoritmo de cálculo de planchas \texttt{boardsNeeded}, el cual estima la cantidad de planchas de melamina de 250$\times$183 cm necesarias para fabricar el mueble usando el área total de las piezas. El resultado se despliega en el componente \texttt{CubicacionPanel}, que lista las piezas agrupadas por módulo con su cantidad y dimensiones, y muestra al final el total de planchas requeridas.\\

% NOTA: Agregar captura de la vista del CubicacionPanel con el listado de piezas
% \begin{figure}[H]
%   \includegraphics[width=1\linewidth]{imagenes/cubicacion_panel.png}
%   \caption{Panel de cubicación con listado de piezas}
% \end{figure}
...
\begin{itemize}
    ...
    \item Las dimensiones de los Laterales corresponden a la profundidad y al alto del mueble.
    \item El Techo/Piso utiliza el ancho y la profundidad del mueble.
    \item Módulo sin estantes ni puertas genera 3 tipos de piezas.
    \item Módulo con estantes genera la pieza Estantes con la cantidad correcta.
    \item Las puertas descuentan correctamente 3 mm del ancho y del alto.
...
\subsubsection{Retrospectiva}
Al final de esta iteración se revisó el peso asignado y la estimación fue ajustada, ya que el algoritmo de generación de piezas requirió más casos de borde de los previstos inicialmente. La iteración se completó con todas las funcionalidades operativas.
...
\subsubsection{Diseño}
El flujo del módulo de cotizaciones comienza cuando el mueblista ha completado el diseño y la cubicación. A partir de los datos calculados en el frontend (lista de cortes, resumen de materiales y precio total), se envía una petición POST al backend para crear y persistir la cotización en la base de datos. Luego el sistema permite previsualizarla y exportarla como PDF.

\subsubsection{Implementación}
Para el backend se creó el modelo de datos \texttt{cotizacion.js} en MongoDB, el cual almacena el identificador del mueblista, la referencia al diseño de mueble, el nombre y correo del cliente, el precio total, la lista de cortes y el resumen de materiales usados. También se registra el estado de la cotización, que puede ser Pendiente, En Proceso o Completado.\\

Se implementó el archivo \texttt{cotizacionRoutes.js} con las rutas correspondientes: GET para listar las cotizaciones del usuario autenticado ordenadas por fecha, GET por ID para obtener una cotización específica, POST para crear una nueva cotización validando que el diseño de mueble exista y pertenezca al usuario, PATCH para actualizar el estado de la cotización y DELETE para eliminarla. Todas las rutas están protegidas por el middleware de autenticación JWT.\\

Para la exportación a PDF se implementó el servicio \texttt{pdfCotizacion.js} en el frontend usando la librería jsPDF. El documento generado incluye encabezado con logo, datos del mueblista y del cliente, tabla con el listado de cortes, resumen de materiales con sus costos y el precio total de la cotización. El estado de la cotización se representa visualmente con un color distintivo según sea Pendiente, En Proceso o Completado.\\

% NOTA: Agregar captura de la previsualización de la cotización y/o del PDF generado
% \begin{figure}[H]
%   \includegraphics[width=1\linewidth]{imagenes/cotizacion_preview.png}
%   \caption{Previsualización de cotización}
% \end{figure}
...
\subsubsection{Retrospectiva}
Al final de esta iteración se revisó el peso asignado y el esfuerzo fue el esperado. La iteración se completó sin pendientes, con todas las funcionalidades del módulo de cotizaciones operativas.
...
\subsubsection{Diseño}
El módulo de biblioteca de materiales permite al mueblista mantener un catálogo digital propio. Cada material tiene un nombre, categoría, precio y atributos específicos según la categoría. Las categorías disponibles son: \textbf{material} (planchas como melamina, con tipo y dimensiones), \textbf{accesorio} (herrajes, con tipo de accesorio), \textbf{tapa-canto} (cantos de PVC o aluminio) y \textbf{cubierta}. Estos materiales quedan vinculados al cálculo de costos en el módulo de cotizaciones.
...
En el backend se implementó el archivo \texttt{materialRoutes.js} con las rutas para listar, crear, actualizar y eliminar materiales, todas protegidas con el middleware de autenticación. El listado soporta filtros por categoría y tipo, lo que permite al frontend cargar solo los materiales necesarios para cada contexto. Se implementó también la función de validación \texttt{validateMaterialPayload} que verifica que los campos obligatorios estén presentes según la categoría del material, por ejemplo que un material de tipo 'accesorio' tenga el campo \texttt{accesorio\_tipo}.\\

En el frontend se creó el componente \texttt{MaterialLibrary.jsx}, que muestra los materiales agrupados por categoría, con opciones para agregar nuevos materiales mediante un formulario, editar los existentes y eliminarlos con un diálogo de confirmación.\\

% NOTA: Agregar captura de la vista de la biblioteca de materiales
% \begin{figure}[H]
%   \includegraphics[width=1\linewidth]{imagenes/material_library.png}
%   \caption{Biblioteca de materiales}
% \end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{2cm}|p{2cm}|p{7cm}|}
        ...
        \textbf{HU} & \textbf{Tarea} & \textbf{Descripción}\\
        \hline
HU16, HU17 & T01 & Crear diagrama de secuencia del módulo de estadísticas\\
\hline
HU16 & T02 & Implementar los KPIs del panel de estadísticas\\
\hline
HU16 & T03 & Implementar los gráficos del panel de estadísticas\\
\hline
HU17 & T04 & Implementar el filtro por año\\
\hline
HU16, HU17 & T05 & Creación y testeo de pruebas de caja negra\\
        \hline
    \end{tabular}
\end{table}
...
Para la visualización se implementaron gráficos de barras horizontales para mostrar los diseños creados por mes, la distribución de cotizaciones por estado (Pendiente, En Proceso, Completado) y los tipos de módulos más utilizados en los diseños. Cuando el usuario no tiene datos registrados, los KPIs muestran el valor 0 y los gráficos muestran un mensaje de 'Sin datos'. El filtro de año permite al mueblista comparar su desempeño entre distintos periodos.\\

% NOTA: Agregar captura del panel de estadísticas con KPIs y gráficos
% \begin{figure}[H]
%   \includegraphics[width=1\linewidth]{imagenes/stats_panel.png}
%   \caption{Panel de estadísticas}
% \end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{2cm}|p{2cm}|p{7cm}|}
        ...
        \textbf{HU} & \textbf{Tarea} & \textbf{Descripción}\\
        \hline
HU13 & T01 & Configurar plantilla de correo en Email.js\\
\hline
...
\subsubsection{Diseño}
El flujo de envío de correo comienza cuando el mueblista ingresa el correo del cliente en la vista de cotización y presiona el botón de enviar. El frontend llama al servicio Email.js con los datos de la cotización usando una plantilla predefinida en la plataforma de Email.js, la cual incluye el resumen de la cotización y el precio total. No se requiere configurar ni mantener un servidor de correo propio ya que Email.js actúa como intermediario.

\subsubsection{Implementación}
Para la implementación de esta funcionalidad se utilizó la librería \texttt{@emailjs/browser} en el frontend. Se creó una plantilla de correo en la plataforma de Email.js que incluye los datos del cliente, el detalle del mueble cotizado, el listado de materiales y el precio total. Desde la vista de cotizaciones, cuando el mueblista ingresa el correo del cliente y confirma el envío, se llama a la función \texttt{emailjs.send} con los parámetros correspondientes de la plantilla. En caso de éxito se notifica al usuario con un mensaje de confirmación, y en caso de error se muestra el mensaje correspondiente.\\

Se eligió Email.js ya que su plan gratuito incluye 200 correos mensuales con 2 plantillas, lo cual es suficiente para las dimensiones del proyecto y no requiere configuración de servidor.\\

% NOTA: Agregar captura de la funcionalidad de envío de correo desde la cotización
% \begin{figure}[H]
%   \includegraphics[width=1\linewidth]{imagenes/email_envio.png}
%   \caption{Envío de cotización por correo}
% \end{figure}
...
\subsubsection{Retrospectiva}
Al final de esta iteración se revisó el peso asignado y el esfuerzo fue menor al estimado, ya que Email.js simplificó bastante la integración. La iteración se completó sin pendientes.

\subsection{Iteración 8}
En esta iteración la historia de usuario a trabajar es HU03: Como usuario quiero crear, guardar, cargar y eliminar el diseño de un mueble. Si bien las rutas del backend para guardar y cargar muebles fueron implementadas en la Iteración 2, en esta iteración se desarrolló la vista dedicada de biblioteca de muebles, que permite al mueblista acceder de forma centralizada a todos sus diseños guardados, cargarlos en el canvas o eliminarlos.

\subsubsection{Inicio de Iteración}
En esta etapa, se crean y registran las tareas acordes a las historias de usuario a trabajar, en donde en el cuadro 4.8 se ilustran las tareas resultantes para la iteración.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{2cm}|p{2cm}|p{7cm}|}
        ...
        \textbf{HU} & \textbf{Tarea} & \textbf{Descripción}\\
        \hline
HU03 & T01 & Diseñar la vista de biblioteca de muebles\\
\hline
HU03 & T02 & Implementar la vista con listado de diseños guardados\\
\hline
HU03 & T03 & Implementar la carga de un diseño desde la biblioteca al canvas\\
\hline
HU03 & T04 & Implementar la eliminación de diseños desde la biblioteca\\
\hline
HU03 & T05 & Creación y testeo de pruebas de caja negra\\
        \hline
    \end{tabular}
\end{table}

\subsubsection{Diseño}
La biblioteca de muebles es una vista que lista todos los diseños guardados por el mueblista, mostrando el nombre y la fecha de creación de cada uno. Desde esta vista el mueblista puede seleccionar un diseño para cargarlo en el canvas y continuar editándolo, o eliminarlo con un diálogo de confirmación. La integración con el backend reutiliza las rutas de diseños ya implementadas en la Iteración 2 (GET para listar, DELETE para eliminar).

\subsubsection{Implementación}
Para esta iteración se integró la vista de biblioteca de muebles en la interfaz principal de la aplicación. Al acceder a esta sección, se hace una petición GET al endpoint de diseños del usuario, obteniendo el listado completo ordenado por fecha de creación. Cada diseño se muestra como una tarjeta con el nombre del mueble y la fecha en que fue guardado.\\

Al presionar el botón de cargar, las figuras del diseño seleccionado se deserializan y se cargan en el canvas de Konva, reemplazando el diseño actual. Para la eliminación se muestra un diálogo de confirmación antes de ejecutar la petición DELETE al backend, para evitar eliminaciones accidentales. Una vez eliminado el diseño, la biblioteca se actualiza automáticamente.\\

% NOTA: Agregar captura de la vista de biblioteca de muebles
% \begin{figure}[H]
%   \includegraphics[width=1\linewidth]{imagenes/biblioteca_muebles.png}
%   \caption{Vista de biblioteca de muebles}
% \end{figure}

\subsubsection{Sistema de pruebas}
Después de la implementación, se continúa con las pruebas de la iteración en donde se evalúan los siguientes casos:
\begin{itemize}
    \item Cargar la biblioteca con diseños guardados y verificar que se listan correctamente.
    \item Seleccionar un diseño y cargarlo en el canvas.
    \item Verificar que el canvas muestra correctamente el diseño cargado.
    \item Eliminar un diseño confirmando la acción.
    \item Cancelar la eliminación de un diseño.
    \item Verificar que la biblioteca se actualiza tras una eliminación.
    \item Acceder a la biblioteca con el usuario sin diseños guardados.
\end{itemize}

\subsubsection{Retrospectiva}
Al final de esta iteración se revisó el peso asignado y el esfuerzo fue el esperado, ya que la mayor parte de la lógica de backend ya estaba implementada. La iteración se completó sin pendientes, cerrando así el ciclo de desarrollo de los módulos principales de la aplicación.
...
\begin{itemize}
    ...
    \item \textbf{OE8 — Evaluación de usabilidad:} Se cuenta con el siguiente resultado en la encuesta SUS aplicada al mueblista:
\end{itemize}
...
Suma de ítems impares: (5 + 4 + 3 + 5 + 4) – 5 = 16\\
Suma de ítems pares: 25 – (2 + 1 + 1 + 1 + 2) = 18\\
\textbf{Puntaje SUS: (16 + 18) × 2{,}5 = 85}\\

El puntaje obtenido de 85 puntos se ubica en la categoría \textit{Aceptable} de la escala SUS (mayor a 68), lo que valida que la interfaz es intuitiva y adecuada para el perfil del mueblista artesanal, cumpliendo así el OE8.
...
\begin{itemize}
    \item Mejorar el comportamiento de la detección de colisiones entre figuras del canvas, especialmente para módulos como puertas y cubiertas que tienen lógica de posicionamiento distinta.
    \item Explorar un algoritmo de optimización de corte más avanzado (por ejemplo, un algoritmo de bin packing 2D) para reducir el desperdicio de material en el cálculo de planchas.
    ...
    \item Evaluar la escalabilidad del sistema para soportar múltiples usuarios de forma simultánea.
\end{itemize}
