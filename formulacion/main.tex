...
\item No se considera la implemetación de una aplicación nativa móvil, debido a que módulos como el de cotizaciones para generar el presupuesto está más pensado para trabajarse en un computador antes que en un teléfono.
...
\hline
HU12 & Mueblista & Como usario quiero que el sistema permita enviar cotizaciones por correo\\
\hline
HU13 & Mueblista & Como usuario quiero disponer de una biblioteca de materiales\\
\hline
HU14 & Mueblista & Como ususario quiero poder agregar, editar y eliminar materiales de la biblioteca\\
\hline
HU15 & Mueblista & Como usuario quiero ver estadíscas básicas \\
\hline
...
\hline
Iteración 6 & HU15, HU16 & Implementar estádisticas generales.\\
\hline
...
\end{figure}
En la siguiente figura se muestra el resutados de los metodos anteriormente mencionados utilizando Jest ~\cite{jest} que es un framework de pruebas de javascript para validar las respuestas de las rutas creadas.  
\begin{figure}[H]
...
En la siguiente figura se muestra el algoritmo encargado de generar las piezas para el módulo de cajonera, en donde a partir de las dimensiones del módulo se calculan automáticamente el techo/piso, los laterales, el fondo y el frente según la cantidad de cajones configurada.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it3 1.png}
   \caption{Listado de piezas para cajonera}
\end{figure}

De manera similar, en la siguiente figura se muestra el algoritmo que genera las piezas para el módulo modular, en donde se calculan el techo/piso, los laterales, el fondo, los estantes y las puertas según los valores configurados por el mueblista.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it3 2.png}
   \caption{Listado de piezas para modular}
\end{figure}

Una vez generadas las piezas de todos los módulos del diseño, en la siguiente figura se muestra la función encargada de evaluar la mejor estrategia de ordenamiento para ingresarlas al algoritmo MAXRECTS, en donde se prueba colocarlas sin rotación, con rotación y con una rotación inteligente que prioriza la pieza con el lado más largo alineado al espacio disponible, para así minimizar el desperdicio de material.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it3 3.png}
   \caption{Algoritmo que evalua la estrategia de orden}
\end{figure}

En la siguiente figura se muestra la función central del algoritmo MAXRECTS que posiciona cada pieza dentro de la plancha, en donde se utiliza el criterio BSSF (Best Short Side Fit) para seleccionar el rectángulo libre con el mejor ajuste, para así reducir al máximo el espacio que queda inutilizado en la plancha.
\begin{figure}[H]
...
\begin{itemize}
    ...
    \item Las puertas descuentan correctamente 3 mm del ancho y del alto (para dar espacio al moviemiento de abrir y cerrar la puerta).
...
En la siguiente figura se muestra la ruta POST encargada de crear una nueva cotización, en donde se valida que el diseño de mueble pertenezca al mueblista autenticado antes de guardarlo en la base de datos con estado inicial Pendiente.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it4 1.png}
   \caption{Ruta POST para crear cotización}
\end{figure}

En la siguiente figura se muestra la ruta PATCH para cambiar el estado de una cotización, en donde se validan los estados permitidos (Pendiente, En Proceso y Completado) y se registra la fecha de término cuando se alcanza el estado Completado.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it4 2.png}
   \caption{Ruta PATCH para actualizar estado de cotización}
\end{figure}

En la siguiente figura se muestra la ruta DELETE para eliminar una cotización, en donde se verifica que el documento exista y pertenezca al usuario antes de proceder con la eliminación.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it4 3.png}
   \caption{Ruta DELETE para eliminar cotización}
\end{figure}

En la siguiente figura se muestra la sección del servicio de generación de PDF que construye la tabla de materiales, en donde se recorre el resumen de materiales de la cotización y se agregan las filas con nombre, cantidad y precio unitario usando la librería jsPDF~\cite{jspdf}.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it4 4.png}
   \caption{Tabla de materiales en la generación del PDF}
\end{figure}

Como prueba de las rutas de las cotizaciones también utilizamos Jest para ver el correcto funcionamiento de rutas y no hayan problemas en la carga y descarga de información.
\begin{figure}[H]
...
\subsubsection{Retrospectiva}
Al final de esta iteración se revisó el peso asignado y el esfuerzo fue el esperado. La iteración se completó sin pendientes.
...
\end{figure}
\subsubsection{Implementación}
Se creó el modelo de datos \texttt{material.js} en MongoDB, que almacena para cada material el identificador del usuario, el nombre, la categoría, el precio, la unidad de medida, el grosor, el tipo y los atributos específicos de cada categoría.\\

En la siguiente figura se muestra la función de validación del payload que se ejecuta antes de guardar un material, en donde se verifica que los campos obligatorios estén presentes y que los atributos específicos de cada categoría coincidan con lo requerido, como el tipo de accesorio para accesorios o el ancho y alto para materiales de plancha.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it5 1.png}
   \caption{Validación del payload de material por categoría}
\end{figure}

En la siguiente figura se muestra la inicialización del componente MaterialLibrary, en donde se definen las props que recibe desde el componente padre y el estado inicial que controla el formulario de creación, la selección activa y el modo de edición.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it5 2.png}
   \caption{Estado inicial y props del componente MaterialLibrary}
\end{figure}
Como prueba de las rutas de los materiales también utilizamos Jest para ver el correcto funcionamiento de estas rutas y que no cuenten con errores en los métodos.
\begin{figure}[H]
...
\end{figure}
\subsubsection{Implementación}
Se implementó el componente \texttt{StatsPanel.jsx} en el frontend, que es el encargado de centralizar toda la lógica y visualización de estadísticas. Al cargar el componente se obtienen las cotizaciones del usuario mediante el servicio \texttt{listCotizaciones} y los diseños mediante el servicio de muebles. A partir de estos datos se calculan los siguientes KPIs: total de diseños guardados, total de cotizaciones, monto acumulado de cotizaciones, precio promedio y total de módulos usados.\\
...
En la siguiente figura se muestra el componente BarChart, en donde se implementa la visualización de barras horizontales que es reutilizado para mostrar distintas distribuciones de datos en el panel de estadísticas.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it6 1.png}
   \caption{Componente BarChart para visualización de estadísticas}
\end{figure}

En la siguiente figura se muestra la lógica del filtro por período, en donde a partir del año seleccionado se filtran las cotizaciones y diseños para que todos los indicadores y gráficos del panel muestren únicamente los datos del período elegido.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it6 2.png}
   \caption{Lógica de filtro por período en StatsPanel}
\end{figure}

En la siguiente figura se muestra el cálculo de los KPIs agrupados por mes, en donde se recorren las cotizaciones filtradas y se acumulan los valores de precio, estado y tipo de módulo para construir los datos que alimentan los gráficos del panel.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it6 3.png}
   \caption{Cálculo de KPIs por mes en StatsPanel}
\end{figure}
...
\end{itemize}
\subsubsection{Retrospectiva}
Al final de esta iteración se revisó el peso asignado y el esfuerzo fue el esperado. El módulo de estadísticas quedó operativo con todas sus funcionalidades.

\subsection{Iteración 7}
En esta iteración la historia de usuario a trabajar es HU13: Como usuario quiero que el sistema permita enviar cotizaciones por correo. Esta iteración corresponde a la implementación del envío de cotizaciones por correo electrónico al cliente, integrando el servicio Nodemailer directamente desde el frontend.
...
\subsubsection{Diseño}
El flujo de envío de correo comienza cuando el mueblista ingresa el correo del cliente en la vista de cotización y presiona el botón de enviar. El frontend llama al servicio Nodemailer con los datos de la cotización, la cual incluye el resumen de la cotización y el precio total.
\begin{figure}[H]
...
En la siguiente figura se muestra la configuración del transporter de Nodemailer, en donde se implementaron dos modos de operación: uno con credenciales SMTP reales para producción y otro con una cuenta de prueba de Ethereal para el ambiente de desarrollo, para así no depender de credenciales reales durante el testeo.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it7 1.png}
   \caption{Configuración del transporter Nodemailer con modos SMTP}
\end{figure}

En la siguiente figura se muestra la ruta POST encargada de construir y enviar el correo con la cotización adjunta, en donde se recibe el PDF codificado en base64 desde el frontend y se adjunta al mensaje junto con el nombre del proyecto y el precio total antes de enviarlo mediante el transporter configurado.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it7 2.png}
   \caption{Ruta POST para envío de correo con PDF adjunto}
\end{figure}
Como prueba de las rutas del correo también utilizamos Jest para ver el correcto funcionamiento de estas.
\begin{figure}[H]
...
\subsubsection{Diseño}
La arquitectura de integración centraliza toda la orquestación. La navegación entre módulos se implementa mediante un estado que identifica las dimensiones de la pantalla con cinco pestañas: Diseño, Cubicación, Biblioteca, Estadísticas y Cotizaciones.\\
\begin{figure}[H]
...
\subsubsection{Implementación}
Se modificó para centralizar la orquestación de todos los módulos. El estado que controla qué panel se muestra, las pestañas se renderizan como botones en la barra superior de la vista de escritorio y como fila de botones compactos en móvil.\\

En la siguiente figura se muestra el listener de resize implementado en App.jsx, en donde se detecta el ancho del viewport y se actualiza el estado para activar el layout móvil cuando el ancho es igual o menor a 600 píxeles, para así adaptar la interfaz al dispositivo del mueblista.
\begin{figure}[H]
   \includegraphics[width=1\linewidth]{imagenes/it8 1.png}
   \caption{Listener de resize para layout responsivo en App.jsx}
\end{figure}
...
El segundo patrón son las etiquetas de formulario faltantes, que se concentran en las vistas con campos de entrada.Donde no tienen un elemento '<label>' explícitamente asociado mediante el atributo 'for', lo que puede afectar la experiencia de usuarios que utilizan lectores de pantalla.

Las capturas individuales de la evaluación WAVE para cada vista del sistema se encuentran en el Anexo C.

\chapter{Pruebas}
\label{cap:pruebas}
En este capítulo se presentan los resultados de las pruebas aplicadas al sistema Amedida a lo largo de las ocho iteraciones de desarrollo. Se presentan pruebas de caja negra, que verifican el comportamiento funcional del sistema desde la perspectiva del usuario final, junto con pruebas de aceptación y la evaluación de usabilidad. Los 60 casos de prueba de caja negra PCN-01 a PCN-60 cubren la totalidad de las iteraciones.


\section{Pruebas de Caja Negra}
Al término de cada iteración se ejecutaron pruebas de caja negra sobre el sistema desplegado, evaluando su comportamiento desde la perspectiva del mueblista sin acceder al código fuente. Los 60 casos PCN-01 a PCN-60 se agrupan por iteración y registran la acción realizada, la respuesta del sistema y si ese resultado coincide con lo esperado.
...
\begin{figure}[H]
  ...
  \caption{Pruebas de Caja Negra Iteración 1}
  \label{fig:PN-IT1}
\end{figure}

\subsection{Iteración 2 — Canvas y Diseño (HU02 -- HU06)}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{imagenes/PN it2.png}
  \caption{Pruebas de Caja Negra Iteración 2}
  \label{fig:PN-IT2}
\end{figure}

\subsection{Iteración 3 — Cubicación (HU07, HU08)}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{imagenes/PN it3.png}
  \caption{Pruebas de Caja Negra Iteración 3}
  \label{fig:PN-IT3}
\end{figure}

\subsection{Iteración 4 — Cotizaciones (HU09, HU10, HU12)}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{imagenes/PN it4.png}
  \caption{Pruebas de Caja Negra Iteración 4}
  \label{fig:PN-IT4}
\end{figure}

\subsection{Iteración 5 — Biblioteca de Materiales (HU14, HU15)}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{imagenes/PN it5.png}
  \caption{Pruebas de Caja Negra Iteración 5}
  \label{fig:PN-IT5}
\end{figure}

\subsection{Iteración 6 — Estadísticas (HU15, HU16)}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{imagenes/PN it6.png}
  \caption{Pruebas de Caja Negra Iteración 6}
  \label{fig:PN-IT6}
\end{figure}

\subsection{Iteración 7 — Envío de Correo (HU13)}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{imagenes/PN it7.png}
  \caption{Pruebas de Caja Negra Iteración 7}
  \label{fig:PN-IT7}
\end{figure}

\subsection{Iteración 8 — Navegación e Integración}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{imagenes/PN it8.png}
  \caption{Pruebas de Caja Negra Iteración 8}
  \label{fig:PN-IT8}
\end{figure}

\section{Pruebas de Aceptación}
Las pruebas de aceptación se ejecutaron sobre el sistema desplegado al cierre de cada iteración, recorriendo flujos reales del mueblista. Para cada caso se registran los pasos seguidos, el resultado esperado y lo que efectivamente ocurrió.

\begin{table}[H]
\centering
\caption{Prueba de aceptación PA-01 — Iteración 1: Autenticación}
\label{tab:pa01}
\begin{tabular}{|p{4cm}|p{9.5cm}|}
\hline
\textbf{Test Case ID} & PA-01 \\
\hline
...
\begin{table}[H]
\centering
\caption{Prueba de aceptación PA-02 — Iteración 2: Canvas y Diseño}
\label{tab:pa02}
\begin{tabular}{|p{4cm}|p{9.5cm}|}
\hline
\textbf{Test Case ID} & PA-02 \\
\hline
...
\begin{table}[H]
\centering
\caption{Prueba de aceptación PA-03 — Iteración 3: Cubicación}
\label{tab:pa03}
\begin{tabular}{|p{4cm}|p{9.5cm}|}
\hline
\textbf{Test Case ID} & PA-03 \\
\hline
...
\begin{table}[H]
\centering
\caption{Prueba de aceptación PA-04 — Iteración 4: Cotizaciones}
\label{tab:pa04}
\begin{tabular}{|p{4cm}|p{9.5cm}|}
\hline
\textbf{Test Case ID} & PA-04 \\
\hline
...
\begin{table}[H]
\centering
\caption{Prueba de aceptación PA-05 — Iteración 5: Biblioteca de Materiales}
\label{tab:pa05}
\begin{tabular}{|p{4cm}|p{9.5cm}|}
\hline
\textbf{Test Case ID} & PA-05 \\
\hline
...
\begin{table}[H]
\centering
\caption{Prueba de aceptación PA-06 — Iteración 6: Estadísticas}
\label{tab:pa06}
\begin{tabular}{|p{4cm}|p{9.5cm}|}
\hline
\textbf{Test Case ID} & PA-06 \\
\hline
...
\begin{table}[H]
\centering
\caption{Prueba de aceptación PA-07 — Iteración 7: Envío de Correo}
\label{tab:pa07}
\begin{tabular}{|p{4cm}|p{9.5cm}|}
\hline
\textbf{Test Case ID} & PA-07 \\
\hline
...
\begin{table}[H]
\centering
\caption{Prueba de aceptación PA-08 — Iteración 8: Navegación e Integración}
\label{tab:pa08}
\begin{tabular}{|p{4cm}|p{9.5cm}|}
\hline
\textbf{Test Case ID} & PA-08 \\
\hline
...
A continuación se muestran las respuestas del mueblista:

\begin{enumerate}
    \item Creo que me gustaría utilizar este sistema con frecuencia = 5
    \item Encontré el sistema innecesariamente complejo = 2
    \item Pensé que el sistema era fácil de usar = 4
    \item Creo que necesitaría el apoyo de un técnico para poder utilizar este sistema = 1
    \item Encontré que las diversas funciones de este sistema estaban bien integradas = 3
    \item Pensé que había demasiada inconsistencia en este sistema = 1
    \item Me imagino que la mayoría de la gente aprendería a utilizar este sistema muy rápidamente = 5
    \item Encontré el sistema muy complicado de usar = 1
    \item Me sentí muy seguro usando el sistema = 4
    \item Necesitaba aprender muchas cosas antes de empezar con este sistema = 2
\end{enumerate}

Para calcular el puntaje, a cada ítem impar se le resta 1 y cada ítem par se descuenta de 5, para luego sumar ambos grupos y multiplicar el resultado por 2,5:

\begin{itemize}
    \item Suma de ítems impares: (5 + 4 + 3 + 5 + 4) -- 5 = 16
    \item Suma de ítems pares: 25 -- (2 + 1 + 1 + 1 + 2) = 18
    \item \textbf{Puntaje SUS: (16 + 18) $\times$ 2{,}5 = 85}
\end{itemize}

El resultado fue de 85 puntos, lo que corresponde a la categoría \textit{Aceptable} de la escala SUS (mayor a 68), validando que la interfaz es intuitiva para el perfil del mueblista artesanal y cumpliendo así el OE8.

\section{Resumen del capítulo}
Las pruebas cubrieron tres categorías. Los 60 casos de caja negra PCN-01 a PCN-60 evaluaron el sistema desde la perspectiva del mueblista en cada iteración. Las ocho pruebas de aceptación PA-01 a PA-08 recorrieron los flujos completos al cierre de cada iteración. Finalmente, la prueba de usabilidad SUS arrojó un puntaje de 85 puntos, confirmando que la interfaz es aceptable para el usuario objetivo. Todos los casos fueron aprobados.
...
\begin{itemize}
    ...
    \item \textbf{OE6 — Reducción de tiempo:} A evaluar con el mueblista. La automatización de la cubicación y el cálculo de materiales elimina las horas de cálculo manual, por lo que se espera superar la reducción del 15\% propuesta.
    ...
    \item \textbf{OE8 — Evaluación de usabilidad:} Cumplido. Se aplicó la encuesta SUS al mueblista al término del desarrollo, obteniendo un puntaje de 85 puntos, categoría \textit{Aceptable}. El detalle de la prueba se encuentra en la Sección~\ref{sec:sus}.
\end{itemize}
...
A continuación se presentan las historias de usuario definidas para el sistema Amedida, con su descripción, clasificación de prioridad MuSCoW, estimación de esfuerzo en puntos Fibonacci y criterios de aceptación.
...
\textbf{ID} & HU01 \\
\hline
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero iniciar sesión en el sistema. \\
\hline
\textbf{Prioridad (MuSCoW)} & Must Have \\
\hline
\textbf{Estimación (Fibonacci)} & 3 puntos \\
\hline
\textbf{Criterios de aceptación} & El sistema acepta credenciales válidas y genera un token JWT que mantiene la sesión activa.\newline
El sistema rechaza credenciales incorrectas y muestra un mensaje de error.\newline
La sesión persiste hasta que el mueblista cierre sesión explícitamente. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero un canvas 2D para poder hacer diseños. \\
\hline
\textbf{Prioridad (MuSCoW)} & Must Have \\
\hline
\textbf{Estimación (Fibonacci)} & 5 puntos \\
\hline
\textbf{Criterios de aceptación} & El lienzo 2D se muestra en la vista de diseño al iniciar sesión.\newline
El mueblista puede agregar módulos predefinidos al canvas mediante la interfaz.\newline
Los módulos se representan visualmente con sus dimensiones relativas en el lienzo. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero crear, guardar, cargar y eliminar el diseño de un mueble. \\
\hline
\textbf{Prioridad (MuSCoW)} & Must Have \\
\hline
\textbf{Estimación (Fibonacci)} & 3 puntos \\
\hline
\textbf{Criterios de aceptación} & El mueblista puede guardar el diseño actual con un nombre identificador.\newline
El sistema almacena el diseño en la base de datos y lo lista en la sección de diseños recientes.\newline
El mueblista puede cargar un diseño guardado y el canvas se restablece al estado previo.\newline
El mueblista puede eliminar un diseño de la lista de diseños guardados. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero que el diseño se pueda hacer modular. \\
\hline
\textbf{Prioridad (MuSCoW)} & Must Have \\
\hline
\textbf{Estimación (Fibonacci)} & 8 puntos \\
\hline
\textbf{Criterios de aceptación} & El mueblista puede agregar distintos tipos de módulos: cajonera, modular, estante, base, divisor y cubierta.\newline
Los módulos pueden combinarse y apilarse dentro del canvas.\newline
Cada módulo se renderiza con su forma y proporciones correspondientes. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero que las medidas de los módulos agregados puedan ser editables. \\
\hline
\textbf{Prioridad (MuSCoW)} & Must Have \\
\hline
\textbf{Estimación (Fibonacci)} & 3 puntos \\
\hline
\textbf{Criterios de aceptación} & El mueblista puede seleccionar un módulo y editar sus dimensiones (ancho, alto, profundidad).\newline
El canvas actualiza la representación visual del módulo al confirmar los nuevos valores.\newline
Las dimensiones editadas se reflejan correctamente en el cálculo de cubicación. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero que los módulos sean móviles para acomodarlos según la necesidad. \\
\hline
\textbf{Prioridad (MuSCoW)} & Must Have \\
\hline
\textbf{Estimación (Fibonacci)} & 3 puntos \\
\hline
\textbf{Criterios de aceptación} & El mueblista puede arrastrar módulos dentro del canvas y soltarlos en la posición deseada.\newline
La posición de cada módulo se actualiza en tiempo real durante el arrastre.\newline
Los módulos no se desplazan fuera de los límites del lienzo. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero que el sistema me calcule la cantidad de material. \\
\hline
\textbf{Prioridad (MuSCoW)} & Must Have \\
\hline
\textbf{Estimación (Fibonacci)} & 13 puntos \\
\hline
\textbf{Criterios de aceptación} & El sistema calcula automáticamente las piezas necesarias a partir del diseño del canvas.\newline
Se utiliza el algoritmo MAXRECTS para optimizar la disposición de piezas en los tableros.\newline
El resultado indica el número de tableros necesarios y el porcentaje de aprovechamiento del material. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero que se muestre un listado de las piezas que se van a utilizar. \\
\hline
\textbf{Prioridad (MuSCoW)} & Must Have \\
\hline
\textbf{Estimación (Fibonacci)} & 3 puntos \\
\hline
\textbf{Criterios de aceptación} & El sistema muestra una lista con todas las piezas resultantes de la cubicación.\newline
Cada pieza indica sus dimensiones y a qué tablero pertenece.\newline
La lista se actualiza al modificar el diseño y recalcular la cubicación. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero que el sistema pueda generar cotizaciones a partir del diseño. \\
\hline
\textbf{Prioridad (MuSCoW)} & Should Have \\
\hline
\textbf{Estimación (Fibonacci)} & 5 puntos \\
\hline
\textbf{Criterios de aceptación} & El sistema genera una cotización con el desglose de materiales, mano de obra y precio total.\newline
La cotización toma los datos del diseño activo y los materiales de la biblioteca.\newline
El mueblista puede ingresar los datos del cliente antes de generar la cotización. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero que se vea una previsualización de la cotización. \\
\hline
\textbf{Prioridad (MuSCoW)} & Should Have \\
\hline
\textbf{Estimación (Fibonacci)} & 2 puntos \\
\hline
\textbf{Criterios de aceptación} & El mueblista puede ver una previsualización de la cotización antes de exportarla o enviarla.\newline
La previsualización muestra el desglose de costos y los datos del cliente.\newline
La previsualización refleja el contenido exacto que tendrá el documento PDF generado. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero que las cotizaciones se puedan exportar como PDF. \\
\hline
\textbf{Prioridad (MuSCoW)} & Should Have \\
\hline
\textbf{Estimación (Fibonacci)} & 2 puntos \\
\hline
\textbf{Criterios de aceptación} & El mueblista puede descargar la cotización como archivo PDF con un solo clic.\newline
El PDF incluye los datos del cliente, el desglose de materiales y el precio total.\newline
El archivo generado está correctamente formateado para su presentación al cliente. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero que el sistema permita enviar cotizaciones por correo. \\
\hline
\textbf{Prioridad (MuSCoW)} & Won't Have \\
\hline
\textbf{Estimación (Fibonacci)} & 3 puntos \\
\hline
\textbf{Criterios de aceptación} & El mueblista puede ingresar el correo del cliente y enviar la cotización directamente desde la aplicación.\newline
El sistema confirma el envío exitoso con un mensaje de notificación.\newline
El cliente recibe un correo con la cotización generada en el cuerpo del mensaje. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero disponer de una biblioteca de materiales. \\
\hline
\textbf{Prioridad (MuSCoW)} & Should Have \\
\hline
\textbf{Estimación (Fibonacci)} & 2 puntos \\
\hline
\textbf{Criterios de aceptación} & El mueblista puede ver la lista completa de materiales registrados con sus precios y atributos.\newline
Los materiales están disponibles para ser seleccionados durante la generación de cotizaciones.\newline
La biblioteca persiste entre sesiones y está vinculada al mueblista autenticado. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero poder agregar, editar y eliminar materiales de la biblioteca. \\
\hline
\textbf{Prioridad (MuSCoW)} & Should Have \\
\hline
\textbf{Estimación (Fibonacci)} & 3 puntos \\
\hline
\textbf{Criterios de aceptación} & El mueblista puede agregar un nuevo material especificando nombre, precio y atributos.\newline
El mueblista puede editar los datos de un material existente y guardar los cambios.\newline
El mueblista puede eliminar un material y este desaparece de la biblioteca y de las cotizaciones futuras. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero ver estadísticas básicas. \\
\hline
\textbf{Prioridad (MuSCoW)} & Could Have \\
\hline
\textbf{Estimación (Fibonacci)} & 5 puntos \\
\hline
\textbf{Criterios de aceptación} & El mueblista puede ver un panel de estadísticas con indicadores clave como total de diseños y cotizaciones generadas.\newline
El panel muestra gráficos de barras con el historial de actividad del mueblista.\newline
Las estadísticas se calculan a partir de los datos reales almacenados en la base de datos. \\
\hline
\end{tabular}
\end{table}
...
\textbf{Actor} & Mueblista \\
\hline
\textbf{Historia de usuario} & Como usuario quiero que se puedan filtrar las estadísticas por periodos. \\
\hline
\textbf{Prioridad (MuSCoW)} & Could Have \\
\hline
\textbf{Estimación (Fibonacci)} & 2 puntos \\
\hline
\textbf{Criterios de aceptación} & El mueblista puede seleccionar un período de tiempo (semana, mes, año) para filtrar las estadísticas.\newline
Los gráficos e indicadores se actualizan automáticamente al cambiar el filtro seleccionado.\newline
El filtro refleja correctamente solo los datos correspondientes al rango de tiempo indicado. \\
\hline
\end{tabular}
\end{table}

\appendix{Resultados de Evaluación WAVE}
